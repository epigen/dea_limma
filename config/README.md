...
- annotation (annotation): CSV file with one row per analysis and consisting of 10 mandatory columns
    ...
    -  OvA (optional): Variable names contained in the formula (and metadata) for which a one-vs-all analysis using contrasts should be performed. Separated by `|` (e.g., `cell_type|batch`). A separate result folder `{name}_OvA_{variable}` is generated per `variable`.
